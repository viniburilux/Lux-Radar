...
def _make_record(detail: dict[str, Any], source: dict[str, Any], root: dict[str, Any], evidence: dict[str, Any], candidate: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    normalized = {
        ...
        "fields": {"title": title, "description": record["description"], "organization": record["organization"], "deadline": deadline, "published_at": record.get("published_at"), "updated_at": record["updated_at"], "observed_at": detail["observed_at"], "first_seen_at": record["first_seen_at"], "last_seen_at": record["last_seen_at"], "eligibility": record["eligibility"], "status": status, "official_url": url, "official_pdf_url": pdf_url, "reason_code": "current_opportunity" if status in {"VERIFIED", "OPEN"} else "insufficient_evidence" if status == "INSUFFICIENT_EVIDENCE" else "status_explicitly_closed" if status == "CLOSED" else "missing_status"},
